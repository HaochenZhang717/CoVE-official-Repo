...
for user, interaction in tqdm(interactions.items()):
    ...
    for i in range(len(train_input)):
        if i == 0:
            history_train += "<|" + str(train_input[i]) + "|>: " + "\"" + interaction_titles[i] + "\""
        else:
            history_train += ", <|" + str(train_input[i]) + "|>: " + "\"" + interaction_titles[i] + "\""
    train_target = "<|" + str(train_output) + "|>" + ": " + "\"" + interaction_titles[-3] + "\""
    ...
    for i in range(len(eval_input)):
        if i == 0:
            history_eval += "<|" + str(eval_input[i]) + "|>: " + "\"" + interaction_titles[i] + "\""
        else:
            history_eval += ", <|" + str(eval_input[i]) + "|>: " + " \"" + interaction_titles[i] + "\""
    eval_target = "<|" + str(eval_output) + "|>" + ": " + "\"" + interaction_titles[-2] + "\""
    ...
    with open(f"{args.root}/train.json", 'w') as f:
        json.dump(json_list_train, f, indent=4)

    with open(f"{args.root}/test.json", 'w') as f:
        json.dump(json_list_eval, f, indent=4)

    with open(f"{args.root}/test_input_only.json", 'w') as f:
        json.dump(json_list_eval_input_only, f, indent=4)

    with open(f"{args.root}/test_output_only.json", 'w') as f:
