...
class Trainer:
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # self.save_model(output_dir, _internal_call=True, safe_serialization=False)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        attention_mask = inputs["attention_mask"][..., 1:].contiguous()  # Shift mask to match labels

        # Compute CrossEntropyLoss while ignoring padding tokens
        loss_fct = torch.nn.CrossEntropyLoss(reduction="none")  # No reduction yet
        loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))

        # Apply attention mask
        loss = loss.view(shift_labels.shape) * attention_mask  # Zero out masked tokens
        loss = loss.sum() / attention_mask.sum()  # Normalize over valid tokens only
        print(attention_mask)
        print(f'logits shape: {logits.shape}')
        print(f"my Loss: {loss.item()}, Batch size: {inputs['input_ids'].shape[0]}, Mask sum: {attention_mask.sum().item()}")
        print(f"their Loss: {outputs['loss']}, Batch size: {inputs['input_ids'].shape[0]}")


        causal_loss = ForCausalLMLoss(logits, labels, logits.shape[-1], logits.shape[0])
        print(f"causal loss: {causal_loss}")
        breakpoint()
        ...
        if labels is not None:
            ...
            # Haochen made some changes to make loss not scale with batch size
            if isinstance(outputs, dict):
                loss = outputs["loss"] / inputs[list(inputs.keys())[0]].shape[0]
            else:
